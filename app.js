...
async function loadStandings() {
  ...
  try {
    ...
    list.innerHTML =
      '<div style="padding:12px;color:var(--text3);font-size:11px">Could not load standings.<br>Check API server.</div>';
  }
}
...
async function loadConstructors() {
  ...
  try {
    ...
    el.innerHTML =
      '<div style="color:var(--text3);font-size:10px;padding:4px">—</div>';
  }
}
...
async function loadRaceResults() {
  ...
  try {
    const res = await fetch(
      `${API}/api/race/${state.year}/${state.gp}/results`,
    );
    const data = await res.json();

    const winner = data.find((r) => r.Position == 1);
    const fastest = data.find((r) => r.FastestLap) || null;
    const dnfs = data.filter(
      (r) => r.Status && r.Status !== "Finished" && !r.Status.match(/Lap/),
    );
    const totalLaps = Math.max(...data.map((r) => Number(r.Laps) || 0));

    document.getElementById("stat-winner").textContent =
      winner?.Abbreviation || "—";
    document.getElementById("stat-winner-team").textContent =
      winner?.TeamName || "—";
    document.getElementById("stat-fastest").textContent =
      fastest?.Abbreviation || "—";
    document.getElementById("stat-fastest-time").textContent = Number.isFinite(
      fastest?.FastestLapTime,
    )
      ? fmt(fastest.FastestLapTime)
      : "Fastest lap holder";
    ...
    document.getElementById("resultsBody").innerHTML = data
      .map((r) => {
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            ticks: { color: "#50505f", font: { size: 10 } },
            grid: { color: "#2a2a35" },
          },
          y: {
            title: {
...
async function runStrategyComparison() {
  ...
  try {
    ...
    document.getElementById("stratBody").innerHTML = strategies
      .slice(0, 15)
      .map((s, i) => {
        const delta = s.total_race_time - best;
        const stintHtml = s.stints
          .map(
            (st) =>
    ...
    log(
      `Strategy analysis complete: optimal is ${strategies[0]?.label}`,
      "pit",
    );
  } catch (e) {
    document.getElementById("stratBody").innerHTML =
...
async function runSimulation() {
  ...
  document.getElementById("winProbList").innerHTML =
    '<div class="loading-overlay"><span class="spinner"></span> Simulating…</div>';
  document.getElementById("podiumProbList").innerHTML =
  ...
  try {
    ...
    log(
      `Simulation complete: ${drivers[0]?.driver} most likely to win (${(drivers[0]?.win_probability * 100).toFixed(0)}%)`,
      "pit",
    );
  } catch (e) {
    document.getElementById("winProbList").innerHTML =
      `<div style="color:var(--red);padding:12px">${e.message}</div>`;
...
async function loadTelemetry() {
  ...
  try {
    const res = await fetch(
      `${API}/api/telemetry/${state.year}/${state.gp}/compare?driver_a=${dA}&driver_b=${dB}`,
    );
    const data = await res.json();
        ...
        datasets: [
          {
            ...
            data: delta,
            borderColor: "#9d7df5",
            backgroundColor: delta.map((v) =>
              v > 0 ? "rgba(232,25,44,.15)" : "rgba(79,168,245,.15)",
            ),
            borderWidth: 1.5,
            pointRadius: 0,
            tension: 0.2,
    ...
    log(
      `Telemetry loaded: ${dA} ${fmt(data.lap_time_a)} | ${dB} ${fmt(data.lap_time_b)} | gap: ${fmtDelta(data.gap)}`,
      "pit",
    );
  } catch (e) {
...
async function loadChampionship() {
  const completedRounds =
    state.schedule.length > 0
      ? Math.max(
          0,
          state.schedule.findIndex((r) => r.RoundNumber === state.gp),
        )
      : 0;

  log(
    `Loading ${state.year} championship (${completedRounds} rounds completed)…`,
  );
  ...
  try {
    const res = await fetch(
      `${API}/api/championship/${state.year}?completed_rounds=${completedRounds}&n_simulations=500`,
    );
    const data = await res.json();

    const leader = data.standings[0];
    document
      .getElementById("champLeader")
      .querySelector(".stat-value").textContent = leader?.code || "—";
    document
      .getElementById("champLeader")
      .querySelector(".stat-sub").textContent =
      `${leader?.points} pts · ${leader?.wins} wins`;
    document
      .getElementById("champRounds")
      .querySelector(".stat-value").textContent = data.rounds_remaining;
    document
      .getElementById("champRounds")
      .querySelector(".stat-sub").textContent =
      `Rounds completed: ${completedRounds}`;
    document
      .getElementById("champPts")
      .querySelector(".stat-value").textContent = data.max_points_available;
    document.getElementById("champPts").querySelector(".stat-sub").textContent =
      "Including fastest lap bonuses";

    // Title prob bars
    const topDrivers = data.standings.slice(0, 8);
    const maxProb =
      Math.max(...topDrivers.map((d) => d.title_probability)) || 1;
    document.getElementById("titleProbList").innerHTML = topDrivers
      .map(
        (d) => `
      <div class="prob-bar-row">
    ...
    // Table
    document.getElementById("champBody").innerHTML = data.standings
      .map((d) => {
    ...
    log(
      `Championship loaded: ${leader?.name} leads with ${leader?.points} pts`,
      "pit",
    );
  } catch (e) {
    document.getElementById("champBody").innerHTML =
...
function chartDefaults({ x, y }) {
  return {
    ...
    scales: {
      x: {
        title: { display: true, text: x, color: "#50505f", font: { size: 10 } },
        ticks: { color: "#50505f", font: { size: 10 } },
        grid: { color: "#2a2a35" },
      },
      y: {
        title: { display: true, text: y, color: "#50505f", font: { size: 10 } },
        ticks: { color: "#50505f", font: { size: 10 } },
        grid: { color: "#2a2a35" },
      },
    },
...
document
  .querySelector('[data-view="championship"]')
  .addEventListener("click", loadChampionship);
